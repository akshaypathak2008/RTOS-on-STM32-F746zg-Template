/* DESCRIPTION
 ...
 The  RTOS application involves the following steps:
 
   1. MCU hardware intialization
   2. Defining task functions
   3. Creation & Mapping of tasks to the  functions in step 2.
   4. Starting the RTOS scheduler
...
// GreenTask sleeps for 1.5 secs with LED ON and deletes itself
void GreenTask(void *argument)
{
...
//BlueTask bliks blue LED indefinitely, but cut short due to RedTask, 
void BlueTask( void* argument )
{
	while(1)
	{
...
//RedTask deletes BlueTask on first run, and blinks red LED continuously
void RedTask( void* argument )
{
	uint8_t firstRun = 1;
